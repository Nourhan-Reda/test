/////////////////////////////////Best Selling/////////////////////////////////////////////////////

// دالة لخلط (shuffle) Array
function shuffleArray(array) {
  return array.sort(() => Math.random() - 0.5);
}

// دالة لعرض النجوم
function generateStars(rating) {
  let stars = "";
  const fullStars = Math.floor(rating);
  const halfStar = rating % 1 >= 0.5;

  for (let i = 0; i < fullStars; i++) {
    stars += `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="#ffeb00" d="m12 17.275l-4.15 2.5q-.275.175-.575.15t-.525-.2t-.35-.437t-.05-.588l1.1-4.725L3.775 10.8q-.25-.225-.312-.513t.037-.562t.3-.45t.55-.225l4.85-.425l1.875-4.45q.125-.3.388-.45t.537-.15t.537.15t.388.45l1.875 4.45l4.85.425q.35.05.55.225t.3.45t.038.563t-.313.512l-3.675 3.175l1.1 4.725q.075.325-.05.588t-.35.437t-.525.2t-.575-.15z"/></svg>`;
  }
  if (halfStar) {
    stars += `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="#ffeb00" d="M12.5 2.462c0-.26-.208-.488-.45-.395a1 1 0 0 0-.504.431L8.833 7.175a1.25 1.25 0 0 1-.894.61l-5.086.767c-.855.13-1.154 1.208-.489 1.76l3.79 3.138c.35.29.515.75.43 1.197l-.992 5.205a1 1 0 0 0 1.449 1.072l4.79-2.522q.07-.037.143-.065c.271-.101.527-.322.527-.613z"/></svg>`;
  }
  return stars;
}

// جلب البيانات من JSON
fetch("json/products.json")
  .then(res => res.json())
  .then(products => {
    // تقسيم المنتجات حسب الكاتجوري
    const categories = {};
    products.forEach(p => {
      if (!categories[p.category]) categories[p.category] = [];
      categories[p.category].push(p);
    });

    // ناخد أول 3 منتجات من كل كاتجوري
    let selectedProducts = [];
    Object.values(categories).forEach(cat => {
      selectedProducts = selectedProducts.concat(cat.slice(0, 3));
    });

    const productList = document.getElementById("product-list");
    productList.innerHTML = "";

    selectedProducts.forEach(product => {
      const productHTML = `
        <div class="swiper-slide">
          <div class="bg-light rounded-2 p-3 position-relative product-card">
            <div class="product-actions position-absolute d-flex flex-column" style="top:10px; right:10px;">
              <!-- الأيكونات -->
              <a href="../../wishlist.html?id=${product.id}" class="action-btn d-flex align-items-center justify-content-center rounded-circle bg-white border text-secondary text-decoration-none">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path  d="M4.24 12.25a4.2 4.2 0 0 1-1.24-3A4.25 4.25 0 0 1 7.25 5c1.58 0 2.96.86 3.69 2.14h1.12A4.24 4.24 0 0 1 15.75 5A4.25 4.25 0 0 1 20 9.25c0 1.17-.5 2.25-1.24 3L11.5 19.5zm15.22.71C20.41 12 21 10.7 21 9.25A5.25 5.25 0 0 0 15.75 4c-1.75 0-3.3.85-4.25 2.17A5.22 5.22 0 0 0 7.25 4A5.25 5.25 0 0 0 2 9.25c0 1.45.59 2.75 1.54 3.71l7.96 7.96z"/></svg>
              </a>
            </div>
            <a href="../../product.html?id=${product.id}">
              <img src="assets/${product.images[0]}" class="card-img-top p-3 product-img" alt="${product.name}">
                </a>            
                <div class="fs-6 fw-semibold text-dark mb-2">${product.name}</div>
                <div class="d-flex align-middle mb-2 gap-2">
              <div class="stars d-flex">${generateStars(product.rating)}</div>
              <span class="rating-count">(${product.rating})</span>
            </div>
            <div>
              <span class="fs-5 fw-bold text-danger">${product.price} ${product.currency}</span>
            </div>
            <div class="text-muted small">Stock: ${product.stock}</div>
          </div>
        </div>
      `;
      productList.innerHTML += productHTML;
    });
  })
  .catch(err => console.error("Error loading JSON:", err));




  ///////////////////////////////Explore Our Products///////////////////////////////////////////////////////
fetch("json/products.json")
  .then(response => response.json())
  .then(products => {
    const container = document.getElementById('product-container');
    // Take first 8 products for 2 rows
    const productsToShow = products.slice(0, 8);
    productsToShow.forEach(product => {
      const col = document.createElement('div');
      col.className = 'col-lg-3 col-md-6 col-12 mb-3'; 
      col.innerHTML = `
        <div class="card shadow-sm  position-relative product-card">
          <!-- Icons top-right -->
          <div class="position-absolute top-0 end-0 d-flex flex-column m-2 gap-2 product-actions">
            <a href="../../wishlist.html?id=${product.id}"> <button class="btn btn-light btn-sm rounded-circle d-flex align-items-center justify-content-center action-btn">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                <path d="M4.24 12.25a4.2 4.2 0 0 1-1.24-3A4.25 4.25 0 0 1 7.25 5c1.58 0 2.96.86 3.69 2.14h1.12A4.24 4.24 0 0 1 15.75 5A4.25 4.25 0 0 1 20 9.25c0 1.17-.5 2.25-1.24 3L11.5 19.5zm15.22.71C20.41 12 21 10.7 21 9.25A5.25 5.25 0 0 0 15.75 4c-1.75 0-3.3.85-4.25 2.17A5.22 5.22 0 0 0 7.25 4A5.25 5.25 0 0 0 2 9.25c0 1.45.59 2.75 1.54 3.71l7.96 7.96z"/>
              </svg>
            </button></a>
          </div>
          <!-- Product image -->
        <a href="../../product.html?id=${product.id}"> <img src="assets/${product.images[0]}" class="card-img-top"  alt="${product.name}"></a>
        <a href="../../cart.html?id=${product.id}"><button class="btn btn1 w-100">Add to Cart</button></a>
          <div class="card-body">
            <h6 class="card-title mb-2">${product.name}</h6>
            <p class="mb-2">
              <span class="text-danger fw-bold">${product.currency} ${product.price}</span>
              <span class="text-warning">${'★'.repeat(Math.round(product.rating))}</span>
              <small class="text-muted">(65)</small>
            </p>
          </div>
        </div>
      `;

      container.appendChild(col);
    });
  })
  .catch(error => console.error('Error loading JSON:', error));


////////////////////////Swiper slider////////////////////////////////////////////////

const swiper = new Swiper('.product-slider', {
    // Default settings for large screens (4 slides)
    slidesPerView: 4,
    spaceBetween: 30,
    
    // Responsive breakpoints
    breakpoints: {
        320: {
            slidesPerView: 1,
            spaceBetween: 20,
        },
        768: {
            slidesPerView: 2,
            spaceBetween: 25,
        },
        1024: {
            slidesPerView: 4,
            spaceBetween: 30,
        },
    },

    // Navigation arrows
    navigation: {
        nextEl: '.swiper-button-next',
        prevEl: '.swiper-button-prev',
    },

    pagination: {
        el: ".swiper-pagination",
        clickable: true,
        dynamicBullets: true,       // يعمل النقط Dynamic
        dynamicMainBullets: 4       // يخليهم 4 بس
    },

    // Auto play (optional)
    autoplay: {
        delay: 4000,
        disableOnInteraction: false,
        pauseOnMouseEnter: true,
    },

    // Loop through slides
    loop: true,

    // Smooth transitions
    speed: 600,
    
    // Center slides on smaller screens
    centeredSlides: false,
    
    // Allow users to drag/swipe
    allowTouchMove: true,
    
    // Grab cursor
    grabCursor: true,
});

//  Pause autoplay on hover
const sliderContainer = document.querySelector('.product-slider');
sliderContainer.addEventListener('mouseenter', () => {
    swiper.autoplay.stop();
});
sliderContainer.addEventListener('mouseleave', () => {
    swiper.autoplay.start();
});

        ////////////////////////////////////////////////////////////////////////////
        //nav
        document.addEventListener("DOMContentLoaded", () => {
      fetch("partials/navbar.html")
        .then(res => res.text())
        .then(data => {
          document.getElementById("navbar").innerHTML = data;
        })
        .catch(err => console.error("Error loading navbar:", err));
    });
      // footer
      document.addEventListener("DOMContentLoaded", () => {
      fetch("partials/footer.html")
        .then(res => res.text())
        .then(data => {
          document.getElementById("footer").innerHTML = data;
        })
        .catch(err => console.error("Error loading footer:", err));
    });